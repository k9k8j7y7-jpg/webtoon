...
### 최근 완료 (2026-08-27)
- [x] **P5 — 회차 대본 생성(2-B)** — 구현 + 검증 9/9 PASS + 실전 테스트 통과. 대본 생성/훅 반영/Gate 1 스킵 흐름 모두 정상. 실사용 요구 1건(수정 후 재생성) P6 이월 (6a2e8a7)
...
### 다음 세션 시작점
- **P6 — 1순위: 수정 후 재생성** — 대본 있는 회차의 아웃라인 요약·훅 편집 → 확인 후 기존 대본 대체 재생성. 이미지 있는 회차는 이것도 잠금
- **P6 후속:** 상태 뱃지 정식화, 대본 회차 합치기(아웃라인 병합→재생성, 같은 원리), 잠금 툴팁, 리넘버링 마무리
- **실사용 대기:** "아빠 엄마에 우리 도도" 1화 — 첫 만남을 실화(부모님 댁, 도도가 아내 품에 달려듦)로 수정 후 재생성 예정
