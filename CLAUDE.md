...
- [x] 3단계: 패킷 시스템 구현·배포 완료 (2026-09-10, 커밋 3074373). 생성 5지점 차감(컷1/시트2/장소1/사진변환1/텍스트0) + 잔량 가드(402) + 실패 미차감 + /packets 현황 페이지 + admin_grant.py. user_id=3에 테스트 20패킷 지급됨. 도도 실서버 테스트 후 완료 확정 예정
- [ ] 다음: ① 도도 실서버 패킷 테스트 (잔량 표시 → 버튼 비용 표시 → ep28 컷 재생성 차감 → 부족 차단) → 통과 시 3단계 완료 기록 → ② 4단계(토스페이먼츠) 시작 — 테스트 API 키로 개발, 멱등 처리(중복 지급 방지) 필수
...
- [x] 2층 3단계 패킷 차감 (구현·배포 2026-09-10, 커밋 3074373. 도도 실서버 테스트 후 완료 확정)
- [ ] 2층 4단계 토스페이먼츠 결제 (대기: 3단계 테스트 통과 후)
- [ ] 0-5 도도 4화~ 제작 (병행)
