...
### 현재 상태
- 연작 골격 P1~P6 전체 완성. 실전 시리즈 "아빠 엄마에 우리 도도" 1·2화 대본 완료 (실화 반영)
- 다음 후보 (우선순위 미정 — 사용자와 논의 후 확정):
  1. 도도 시리즈 계속 제작 (3화~ 대본, 이미지 진입) — 실사용이 다음 요구를 알려줌
  2. 연속성 품질 (회차 누적 사실), 존댓말 관계 테이블
  3. 계측 정비 / 모델 비교 실험 / 말풍선 디자인 교체 (대기 트랙)
