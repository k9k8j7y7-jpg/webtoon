...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + 1-8 바이블 수정 + 1-7 지문 AI 재작성 + AI 토큰 상수화 + 0-4 커스텀 폰트 + 캐릭터 이름 편집 + 1-1 장소 사진 업로드 + 인물 배치 상식 + 1-1c 캐릭터 사진→시트(플랜C+참조토글) + 표정 시트 2×3 격자(expressions, 컷 참조, 감정 88→6) + 업로드 검증 개선(Pillow·20MB·리사이즈·동물 체크박스) + 1-4 배경효과 PNG 오버레이(effect_items+카탈로그10종+편집UI+내보내기Base64+배포) + 2층 완공(0~5단계, 2026-09-15) + 이미지 모델 3.1 전환 + 갤러리 좋아요·조회수(step21) + 관리자 페이지 완공(1~5단계, step22+23, 2026-09-15) + 컷 비율 선택(1:1/9:16, step24, 2026-09-16) + 단편 대본 완결 구조 강제(2026-09-16) + 광고 에피소드 1단계(타입·배지·광고 대본, 검증 통과 2026-09-16) + 생성 실패 자동 재시도+안내 개선(2026-09-16) + 광고 2단계(제품 자산, step25, 시트 생성+컷 참조, 2026-09-16) + 광고 3단계(제품 레이어 게이트5+뷰어+내보내기, step26 ENUM, 2026-09-17) + 광고 컷 품질(제품 컷별 선택 has_product+스케일 앵커 프롬프트, 2026-09-17) + 컷 패널 분할 수정(단일 일러스트 강제+장소 참조 샷 분기+다인물 세로 구도, 2026-09-18) + 제품 투명 PNG 전용 업로드+시트 유지+게이트3 읽기 전용+내보내기 PNG 투명 보존(2026-09-18) + 관리자 갤러리 showcase 카테고리 is_ad 자동 결정 버그 수정+AD 배지+ep30 광고 탭 공개(2026-09-19) + v2 1단계: 컷 비율 5종(1:1/9:16/16:9/3:4/4:3)+인스타 letterbox+prompt-preview API(2026-09-20)

**진행 중: 게이트1 개편 — 아이디어 → 기획서** (지시서: `docs/지시서-게이트1-기획서.md`, 단편·광고 대상, 연작 범위 밖)
- [x] 0단계 조사 (2026-09-21)
- [x] 1단계 배포 (2026-09-21) — **검증 완료** (자동 정리·F5 유지·읽기 전용 표시 확인)
  - idea_brief: 모달에서 이야기+제품정보 → gate_status.idea_brief.raw 저장 → Gate1 진입 시 LLM 자동 정리(5칸: 한 줄 소개/등장 캐릭터/기승전결 4칸/톤) → suggested로 장르·분위기·전개 미리 선택 → blur 시 PUT 저장 → "이렇게 바꿔줘" 재정리 → 읽기 전용 표시
  - 광고 모달: 제품명+제품 특징 필드 추가 → idea_brief.product 저장 → Gate3 제품 폼 미리채움
  - WorkflowPage: location.state.idea 전달 제거 → gate_status가 단일 소스
  - 기존 에피소드(idea_brief 없음): 폴백 — 아이디어 정리 카드에 안내 텍스트, 기존 직접 기획 생성 flow 유지
  - 검증 항목: 새 단편 자동 정리 · F5 유지 · "이렇게 바꿔줘" 재정리 · 광고 모달 제품 필드→Gate3 미리채움 · ep30 등 기존 에피소드 회귀
- [x] 2단계 배포 (2026-09-22) — **도도 검증 대기**
  - 2-1 등장인물 미리 채움: idea_brief.characters에 gender/age 추가(프롬프트), charTouched 플래그로 재정리 시 덮어쓰기 방지, [자동 생성] 버튼에 확인 다이얼로그
  - 2-2 장르·분위기·전개 user_touched 세분화: genreTouched/moodTouched/devTouched 개별 관리, 안 바꾼 축만 재정리 시 갱신
  - 2-3 기획 생성에 idea_brief 전체 주입: 프롬프트에 기획서 인물·줄거리·톤·제품 포함 + "새 인물 추가 금지" 명시. 시놉시스 4단 dict(synopsis_parts) 출력+저장+편집+읽기 전용 표시. 제목 덮어쓰기 중단(사용자 제목 유지, AI 추천은 suggested_title로 별도 표시+[적용] 버튼). 대본 생성도 4단 구조 주입
  - 검증 항목: 새 단편에서 기획 생성 → 인물이 idea_brief 것과 일치하는지 · 시놉시스 4단 표시 · 제목 유지 · AI 추천 제목 표시 · 기존 ep 회귀
- [ ] 3단계: 읽기 전용 뷰 보강
...
- 제품 레이어: product_items(spec JSON). ProductLayer 컴포넌트 — 제품 실사 사진 오버레이. CutEditor 드래그·회전·크기·투명도 편집. 레이어 순서: 이미지→제품→효과→말풍선→효과음. 뷰어+내보내기 4경로 Base64 인라인
...
- [~] **게이트1 기획서 개편** — 지시서 `docs/지시서-게이트1-기획서.md`. 1단계 배포·검증 대기(2026-09-21). 2단계=등장인물 미리 채움+기획 생성 idea_brief 주입+시놉시스 4단. 3단계=읽기 전용 보강
