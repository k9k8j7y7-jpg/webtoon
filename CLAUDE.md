...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + P5 ref_key 버그 수정 + 유령 ref_key 정리
...
**도도 시리즈 1화:** 외형 메모 3인 입력 완료, 참조+명세 적용 재생성 통과 (안경·머리·도도 유지 확인). 유령 ref_key(char_0/char_1) 정리됨 — 4컷 수정, #6 husband 추가

**다음 세션 시작점:** 0-4 커스텀 폰트 (조사 지시 있음 — 폰트 체계/export 대기/폭 계산/서빙. 구현 설계: 폰트별 폭 계수 테이블 + bubble-shot 경계 판정. 사용자 폰트 선정 4~6종 대기) 또는 0-5 도도 시리즈 2화 이미지 제작 (사용자 선택)
