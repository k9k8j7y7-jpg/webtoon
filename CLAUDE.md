...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + 1-8 바이블 수정 + 1-7 지문 AI 재작성 + AI 토큰 상수화 + 0-4 커스텀 폰트 + 캐릭터 이름 편집 + 1-1 장소 사진 업로드 + 인물 배치 상식 + 1-1c 캐릭터 사진→시트(플랜C+참조토글) + 표정 시트 2×3 격자(expressions, 컷 참조, 감정 88→6) + 업로드 검증 개선(Pillow·20MB·리사이즈·동물 체크박스) + 1-4 배경효과 PNG 오버레이(effect_items+카탈로그10종+편집UI+내보내기Base64+배포) + 2층 0단계(스키마 설계) + 2층 1단계(소셜 로그인 3사)
...
- [x] 1단계: 소셜 로그인 3사(카카오/구글/네이버) 버튼 + OAuth 콜백 페이지, .env 키 설정 완료
- [ ] 다음: 도도 실서버 3사 로그인 테스트 → 본계정 확정 → 기존 데이터 귀속 마이그레이션(리허설·백업·승인) → 2단계(대시보드)
- 메모: 구글 앱 테스트 모드(개인정보처리방침 URL 등록 후 게시, 2단계에서), 토스 가맹 신청은 3단계 전
...
- [x] 2층 1단계 소셜 로그인 (완료 2026-09-08, OAuth 3사 버튼+콜백+.env 키 설정)
- [ ] 2층 2단계 대시보드 (대기: 로그인 테스트 → 데이터 귀속 후)
- [ ] 0-5 도도 4화~ 제작 (병행)
