...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + P5 ref_key 버그 수정 + 유령 ref_key 정리 + 1-8 바이블 화자/인물 인라인 수정 + 1-7 지문 AI 재작성
...
- 콘티 지문 AI 재작성(1-7): "이렇게 바꿔줘" → AI가 외형 명시 지문 + 캐릭터 제안. 저장 안 함(프론트 채움)

**도도 시리즈:** 1·2화 이미지 완료, 3화 대본 승인·게이트3 진행 중. 1-8로 3화 대본에서 화자(아내, 1인칭) 일관 실증

**바이블⇔DB 캐릭터 이름 불일치 표시:** 처리 완료 (커밋 3ef3029). character_sheets 맵으로 바이블 보기 모드에서 ⚠ 표시

**다음 세션 시작점:**
1. 1-7 실전 — 2화 컷 #2에서 "이렇게 바꿔줘" → 지문 다시 쓰기 → 캐릭터 제안 칩 → 저장 후 재생성
2. 3화 이미지 제작 계속 (참조·명세·화자 전부 맞는 첫 회차 — 손본 컷 수가 성적표)
3. 이후 사용자 선택: 0-4 폰트(폰트 선정 대기, docs/에 후보 폰트 7개) 또는 1-1 장소 사진 업로드
...
- **컷 프롬프트 한글 금지:** 외형은 appearance_en(영어)만 주입. pose 필드는 아직 한글 (추후 영어화 후보)
