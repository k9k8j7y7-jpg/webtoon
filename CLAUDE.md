...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + 1-8 바이블 수정 + 1-7 지문 AI 재작성 + AI 토큰 상수화 + 0-4 커스텀 폰트 + 캐릭터 이름 편집 + 1-1 장소 사진 업로드 + 인물 배치 상식 + 1-1c 캐릭터 사진→시트(플랜C+참조토글) + 표정 시트 2×3 격자(expressions, 컷 참조, 감정 88→6) + 업로드 검증 개선(Pillow·20MB·리사이즈·동물 체크박스) + 1-4 배경효과 PNG 오버레이(effect_items+카탈로그10종+편집UI+내보내기Base64+배포) + 2층 0~4단계(스키마→로그인→대시보드→패킷→결제) + 이미지 모델 3.1 전환 + 2층 5단계 1·2차(랜딩+공개 뷰어+갤러리 실데이터)
...
- [ ] 2층 5단계 마무리: 도도 재확인(시크릿 창 갤러리→뷰어→폰 공유 링크) → 통과 시 2층 공식 완공
- [ ] 폴리싱 백로그: 뷰어 컷 클릭 확대 모달(게이트5 미리보기 스타일), 랜딩 디테일 수정, 광고 샘플 웹툰
- [ ] 다음 공사 후보(도도와 순서 논의): 관리자 페이지 / 3화 잔여+시트 교체+4화 / 오픈 준비(네이버 검수·토스 라이브)
- [ ] 0-5 도도 4화~ 제작 (병행)
- [ ] 후순위: 컷 비율 사용자 선택(1:1/9:16), ep_no 중복 정리, 텍스트 모델(gemini-2.5-flash) 교체(종료일 발표 시)
