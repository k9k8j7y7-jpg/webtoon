...
### 최근 완료 (2026-08-26)
- [x] **P4 — 연작 입구 + 바이블 + 아웃라인** — 백엔드 10 API + SeriesPage + ProjectPage 연작 모달. 검증 27/27 ALL PASS. 실사용 테스트 통과. 실전 시리즈 "아빠 엄마에 우리 도도" 8화 아웃라인 존재 — P5의 첫 실전 재료 (0171dd2, eee782f)
- [x] **P4 보완** — 비동기 진행 표시(오버레이/잠금/하이라이트/재시도) + 모바일 수정모드(제목 넘침/auto-resize/자동완성 차단) (eee782f)
...
### P4 완료 — 다음: P5 (2-B 회차 대본)
- **P5 = 2-B 회차 대본:** 회차 선택 → episodes 행 생성(지연 생성) → 대본 생성(바이블+회차 요약·훅+직전 회차 요약 컨텍스트) → 직전 회차 캐릭터 자동 연결 → Gate 3~5 기존 경로
- **이월:** 자동생성 스킵 배너 실발화 검증 (직전 회차 캐릭터가 대본에 등장하는 케이스)
