...
- [x] 2단계: 대시보드 완료 (2026-09-09, 공지 바·헤더 리뉴얼·패킷 잔량 API·법률 페이지 3종·EziToon 전환·step18 마이그레이션)
- [ ] 다음: 도도가 구글 OAuth 앱 게시 (개인정보처리방침 URL 등록) → 3단계(패킷 차감)
- 메모: 서비스명 EziToon(이지툰) 확정. 네이버 검수는 오픈 준비 때. 토스 가맹 신청은 3단계 전
