...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + 1-8 바이블 수정 + 1-7 지문 AI 재작성 + AI 토큰 상수화 + 0-4 커스텀 폰트 + 캐릭터 이름 편집 + 1-1 장소 사진 업로드 + 인물 배치 상식 + 1-1c 캐릭터 사진→시트(플랜C+참조토글) + 표정 시트 2×3 격자(expressions, 컷 참조, 감정 88→6) + 업로드 검증 개선(Pillow·20MB·리사이즈·동물 체크박스) + 1-4 배경효과 PNG 오버레이(effect_items+카탈로그10종+편집UI+내보내기Base64+배포)
...
**진행 중:** 1-4 배경효과 — 지시서 1~4단계 전부 구현·커밋·배포 완료. 도도 실서버 스팟 체크 대기 (효과 추가→새로고침 유지→내보내기 1경로). 통과 시 1-4 공식 종료

**다음 순서:**
1. 스팟 체크 통과 → 1-4 완료 기록
2. 3화 잔여 컷 정리 (배경효과 실전 사용)
3. 본 캐릭터 시트 교체 (4화 직전, approved 잠금 — smile/angry → expressions 격자 재생성)
4. 4화 제작
...
- [x] 1-4 배경효과 PNG 오버레이 (지시서 1~4단계 구현·배포 완료, 스팟 체크 대기)
- [ ] 0-5 도도 4화~ 제작 (병행)
