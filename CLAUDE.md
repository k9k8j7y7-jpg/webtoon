...
**다음 작업:** 캐릭터 외형 명세 주입 (appearance 필드→프롬프트 명세 블록→안경 유지 검증). 그 다음: 모델 비교 실험 시급도 판단

**실사용:** "아빠 엄마에 우리 도도" 1화 12컷 생성 중. 핵심 문제: 캐릭터 외형 일관성 (안경·머리·복장이 컷마다 흔들림)

## 남은 작업

### 미완료 번들
- [ ] SFX Export 검증 (프론트 SVG 경로로 이미 포함 — 검증만)
- [ ] 긴 대사 문장 단위 분할 + 세로 연결 (D단계)
- [ ] 제품 레퍼런스 업로드 (PPL)
- [ ] 배경 효과 PNG 레이어

### 우선순위 높음
- [ ] OAuth 로그인 UI (Google/Kakao/Naver) — 현재 JWT 직접 입력
- [ ] S3 업로드 연동

### 우선순위 중간
- [ ] Celery + Redis 큐 (서버 재시작 시 Job 소실)
- [ ] 모바일 반응형 UI 최적화

### 우선순위 낮음 (V2)
- [ ] 결제 PG (Toss), 협업, 영상 변환, 어드민 대시보드
