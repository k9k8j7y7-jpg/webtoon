...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + 1-8 바이블 수정 + 1-7 지문 AI 재작성 + AI 토큰 상수화 + 0-4 커스텀 폰트 + 캐릭터 이름 편집 + 1-1 장소 사진 업로드 + 인물 배치 상식 + 1-1c 캐릭터 사진→시트(플랜C+참조토글) + 표정 시트 2×3 격자(expressions, 컷 참조, 감정 88→6) + 업로드 검증 개선(Pillow·20MB·리사이즈·동물 체크박스) + 1-4 배경효과 PNG 오버레이(effect_items+카탈로그10종+편집UI+내보내기Base64+배포) + 2층 0~4단계(스키마→로그인→대시보드→패킷→결제) + 이미지 모델 3.1 전환 + 2층 완공(0~5단계: 스키마→로그인→대시보드→패킷→결제→랜딩+공개 뷰어+갤러리, 2026-09-15)
...
- [ ] 폴리싱 백로그: 좋아요·조회수 프론트(step21 배포 후), 뷰어 컷 클릭 확대 모달(게이트5 미리보기 스타일), 랜딩 디테일 수정, 광고 샘플 웹툰
