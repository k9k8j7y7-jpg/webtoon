...
**전체 완료:** 백엔드 7단계 + 프론트엔드 MVP + 5게이트 전체 + 연작 P1~P6 + 콘티 인라인 수정 + 0-2 appearance_en + P5 ref_key 버그 수정
...
**0-2 완료:** appearance_en 외형 명세 주입 — step13 적용, 백필 25/27, 남편 안경 유지 검증 통과
**P5 버그 수정 완료:** 바이블 ref_key 누락 → 연작 회차 참조 불일치 100%→0%. ep20 1화 기존 12컷은 참조 0건으로 생성된 상태였음 (재생성 필요)

**다음 세션 시작점:** 사용자 실전 — 도도 시리즈 1화 게이트3 외형 메모 3인 입력(남편 안경/아내 머리/도도 갈색+스카프) → 문제 컷 재생성 → 안경 유지 컷 수 = 모델 실험(0-3) 시급도 판단 근거. 컷 #6은 콘티 수정+재생성으로 아내가 안는 장면 교정
...
- **캐릭터 ref_key:** 바이블/기획 생성 시점에 확정 (영문 snake_case). 시트·대본·컷 참조 전부 이 키 기준. 자유 생성 금지
