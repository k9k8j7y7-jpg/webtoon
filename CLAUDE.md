...
- [ ] **다음: 컷 비율 선택(1:1/9:16) 구현** — 조사 완료 (`docs/조사-컷비율선택.md`), 구현 지시 대기. 설계 요점: 게이트3에서 생성 전 선택 + 첫 생성 후 잠금(백엔드 검증 포함), 기존 에피소드 1:1 자동 잠금, 신규 기본 9:16, 장소도 에피소드 비율, 시트는 1:1 유지. aspect_ratio를 Gemini API config에 실제 전달하는 버그 수정 포함
- [ ] 이후 예정: 광고 타입 + 제품 자산 → 광고 샘플 → 3화 잔여 → 시트 교체 → 4화
