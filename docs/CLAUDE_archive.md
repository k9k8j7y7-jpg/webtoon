...
### 게이트5 콘티 인라인 수정 (2026-08-27)

- Gate5 컷 카드에 [콘티] 버튼 → 모달: 샷 타입 4종 선택, 등장 캐릭터 토글(에피소드 캐릭터 목록), 지문 textarea
- 저장만: `PUT /cuts/{cut_id}` (기존 Gate4 API 재사용, 백엔드 수정 없음)
- 저장 후 재생성: PUT → `POST /cuts/{cut_id}/regenerate` 직렬 호출
- 기존 캐릭터 emotion/pose 보존, 새 캐릭터는 neutral 기본값
- "콘티 전체 보기 (게이트 4)" 외부 링크
- 프롬프트 조립 경로: `cuts.spec` → `build_cut_prompt()` (action:61행, shot:75행, characters:27-56행)

### P6 — 수정 후 재생성 + 상태·잠금 정식화 (2026-08-27)

지시서: `docs/지시서-P6-수정재생성-잠금.md`

**백엔드:**
- `POST /series/{sid}/outline/{no}/revise` — 요약·훅 수정 → 대본 재생성 (이미지 409). planning 동기 + series_context 주입
- merge 확장: 대본 회차 병합 시 front ep 재사용 + back ep 소프트삭제 + episode_characters 정리 + 대본 재생성
- `has_images` 집계: `_episode_image_counts()` — `cuts.image_url IS NOT NULL` GROUP BY. outline 응답에 boolean 포함
- split 방어: 대본 회차 분할 409 + "[수정 후 재생성]으로 조정하세요" 안내
- `image_count` list_series 집계

**프론트엔드 (SeriesPage.jsx):**
- 3단계 잠금 UI: outline=자유편집, script=revise만, image=완전잠금(Lock 아이콘+툴팁)
- 상태 뱃지 4종: 아웃라인(회색)/대본(파란+PenLine)/이미지(녹색+Image)/생성중(파란+Loader2)
- revise 편집 모드: 인라인 title/summary/hook 폼 + amber 경고 박스
- advice 배너: revise 후 `next_has_script` 시 다음 회차 갱신 권장
- merge 확인 대화상자: 대본 회차 병합 시 "두 회차의 대본을 합쳐 재생성" 안내
...
### 연작 골격 P1~P6 전체 완성 (2026-08-27)
- [x] **P6 — 수정 후 재생성 + 잠금** — revise API + merge 확장 + 3단계 잠금 + 상태 뱃지 4종. 검증 8/8 + 실전 테스트 통과 (e261753)
- [x] **P5 — 회차 대본 생성(2-B)** — 구현 + 검증 9/9 PASS + 실전 테스트 통과 (6a2e8a7)
- [x] **P4 — 연작 입구 + 바이블 + 아웃라인** — 백엔드 10 API + SeriesPage + ProjectPage. 검증 27/27 (0171dd2, eee782f)
- [x] **P4 보완** — 비동기 진행 표시 + 모바일 수정모드 (eee782f)
- [x] **P3 보완** — 스타일 뱃지/경고, 자동생성 스킵, step12 (a906a4e, e453647)
...
### 현재 상태
- 연작 골격 P1~P6 완성 + 게이트5 콘티 인라인 수정 배포됨 (사용자 실전 검증 완료 — 컷 캐릭터/지문 수정→재생성 루프 작동)
- 다음 세션 시작점: 캐릭터 외형 명세 주입 (외형 일관성 1차 대응 — 지시 초안은 사용자가 보유. 조사→appearance 필드→프롬프트 명세 블록 주입→안경 유지 검증)
- 그 다음 판단: 명세 주입 결과에 따라 모델 비교 실험 (2.5→3.1-flash/Pro) 시급도 결정
- 실사용 현황: "아빠 엄마에 우리 도도" 1화 이미지 12컷 생성, 컷별 수정 진행 중. 발견된 핵심 문제: 캐릭터 외형 일관성 (안경·머리·복장이 컷마다 흔들림 — front 1장 참조의 한계)
