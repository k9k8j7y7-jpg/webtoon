...
## 현재: 1-4 배경효과 PNG 오버레이 완료 (2026-09-07)

### 1-4 배경효과 PNG 오버레이 — 완료 (2026-09-07)
- 카탈로그 10종 (full 6 + partial 4), 수동 배치 (위치·크기·회전·불투명도·반전)
- 내보내기 전 경로 반영 (Base64 인라인), 실서버 스팟 체크 통과 (width>100% 저장 + F5 유지 + ZIP 내보내기)
- 커밋: 3ed0c16(백엔드+카탈로그), d4bb520(편집 UI), 13e3c68(내보내기), 1ddcdf6(width 검증 완화)
- 후순위로 넘긴 것: 자동 추천(감정→효과), 관리자 PNG 업로드
- 다음: 3화(ep26) 잔여 정리 → 본 캐릭터 시트 교체(4화 직전) → 4화 제작

## 이전: 표정 시트 격자화 완료 (2026-09-04)
...
- step14 (reference_photo_url) + step15 (converted_photo_url) 배포
- 인물 배치 상식 (PLACEMENT_COMMON_SENSE) 4곳 배포

## 이전: 0-4 커스텀 폰트 완료 + 1-1 장소 사진 조사 (2026-09-02)
